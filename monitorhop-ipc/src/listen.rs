    ...
    pub async fn new() -> Result<Self, IpcListenerCreationError> {
        ...
        let (socket_path, listener) = {
            ...
            if socket_path.exists() {
                ...
                // of monitorhop is already running
                match UnixStream::connect(&socket_path).await {
                    // connected -> monitorhop is already running
                    Ok(_) => return Err(IpcListenerCreationError::AlreadyRunning),
                    // monitorhop is not running but a socket was left behind
                    Err(e) => {
            ...
            let listener = match UnixListener::bind(&socket_path) {
                Ok(ls) => ls,
                // some other monitorhop instance has bound the socket in the meantime
