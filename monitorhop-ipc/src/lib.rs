...
pub enum IpcListenerCreationError {
    ...
    AlreadyRunning,
    #[error("failed to bind monitorhop socket: `{0}`")]
    Bind(io::Error),
}
...
pub const DEFAULT_PORT: u16 = 4252;
...
pub enum FrontendEvent {
    ...
    /// whether mDNS-SD discovery is on. When true, monitorhop
    /// advertises a `_monitorhop._udp.local.` Bonjour service whose
...
#[cfg(unix)]
const MONITORHOP_SOCKET_NAME: &str = "monitorhop-socket.sock";
...
pub fn default_socket_path() -> Result<PathBuf, SocketPathError> {
    ...
    Ok(Path::new(xdg_runtime_dir.as_str()).join(MONITORHOP_SOCKET_NAME))
}
