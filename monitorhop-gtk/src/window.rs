...
impl Window {
    ...
    fn setup_icon(&self) {
        self.set_icon_name(Some("dev.monitorhop.MonitorHop"));
    }
    ...
    pub(super) fn request_release_bind(&self, chord: Vec<monitorhop_ipc::scancode::Linux>) {
    ...
    pub(super) fn set_release_bind(&self, chord: Vec<monitorhop_ipc::scancode::Linux>) {
    ...
    pub(super) fn set_running_apps(&self, apps: Vec<monitorhop_ipc::RunningApp>) {
    ...
    /// the reconnect path in `monitorhop_gtk::build_ui`.
    ...
    pub(super) fn set_authorized_keys(
        &self,
        mut fingerprints: HashMap<String, monitorhop_ipc::IncomingPeerConfig>,
    ) {
