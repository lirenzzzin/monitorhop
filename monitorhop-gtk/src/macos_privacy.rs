...
pub fn relaunch_bundle() {
    ...
    // at <bundle>/Contents/MacOS/monitorhop, so three parents up is the
...
/// accepts `with icon`, which lets us show the MonitorHop app icon instead
...
fn relaunch_alert_script(icon_icns: Option<&str>) -> String {
    let mut script = String::from(concat!(
        "activate\n",
        "display dialog \"Accessibility access has been granted. MonitorHop ",
...
/// executable (`<bundle>/Contents/MacOS/monitorhop` → three parents up is
...
/// prompt. This is where the familiar first-launch "MonitorHop.app would
...
fn fire_initial_prompts_inner() {
    ...
    // toggle persistently).
    log::info!("ensuring MonitorHop is listed under Input Monitoring");
    unsafe {
    ...
    log::info!("ensuring MonitorHop is listed under Accessibility > Post Event");
    unsafe {
