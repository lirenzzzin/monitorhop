...
pub fn setup(app: &adw::Application, window: &Window) {
    ...
    STATUS_ITEM.with(|item| {
        ...
        unsafe {
            ...
            let menu = menu(&[
                menu_item(c"Open MonitorHop", c"showMonitorHop:"),
                separator_item(),
                menu_item(c"Quit MonitorHop", c"quitMonitorHop:"),
            ]);
            ...
            msg_send_void_id(button, sel(c"setToolTip:"), nsstring(c"MonitorHop"));
...
fn delegate_class() -> Class {
    ...
    *CLASS.get_or_init(|| unsafe {
        ...
        let class_name = CString::new("MonitorHopStatusItemDelegate").unwrap();
        ...
        class_addMethod(
            class,
            sel(c"showMonitorHop:"),
            show_monitorhop as *const c_void,
            c"v@:@".as_ptr(),
        );
        class_addMethod(
            class,
            sel(c"quitMonitorHop:"),
            quit_monitorhop as *const c_void,
            c"v@:@".as_ptr(),
        );
...
extern "C" fn show_monitorhop(_this: Id, _cmd: Sel, _sender: Id) {
    present_window();
}
...
extern "C" fn quit_monitorhop(_this: Id, _cmd: Sel, _sender: Id) {
    STATUS_ITEM.with(|item| {
