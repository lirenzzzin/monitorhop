...
impl Tray for MonitorHopTray {
    fn id(&self) -> String {
        "dev.monitorhop.MonitorHop".into()
    }
    ...
    // (notably waybar on Hyprland) resolve "dev.monitorhop.MonitorHop" to
...
fn render_tray_pixmaps() -> Vec<Icon> {
    ...
    // resources/icons/monitorhop-tray.svg. Tight viewBox + simple
    // silhouette so the glyph remains readable at 12-22 px.
    const SVG_RESOURCE: &str = "/com/monitorhop/MonitorHop/icons/monitorhop-tray.svg";
...
pub(crate) fn setup(app: &adw::Application, window: &Window) -> gio::ApplicationHoldGuard {
    ...
    let service = TrayService::new(MonitorHopTray {
        tx,
        last_toggle: None,
        icon_pixmaps,
    });
