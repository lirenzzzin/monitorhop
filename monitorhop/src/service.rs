use crate::{
    ...
    listen::{ListenerCreationError, MonitorHopListener},
...
pub struct Service {
    ...
    /// shared `PrimaryCache` (read by `MonitorHopConnection`) from
    /// peer announcements.
    discovery: Discovery,
...
impl Service {
    pub async fn new(config: Config) -> Result<Self, ServiceError> {
        ...
        // events arrive.
        let listener =
            MonitorHopListener::new(config.port(), cert.clone(), authorized_keys.clone()).await?;
        let primary_cache: PrimaryCache = Default::default();
        let conn =
            MonitorHopConnection::new(cert.clone(), client_manager.clone(), primary_cache.clone());
    ...
    fn commit_suppression(&mut self, suppression: monitorhop_ipc::ClipboardSuppression) {
        let host = HostKind::current();
