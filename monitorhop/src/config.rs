...
/// in [`monitorhop_proto::ProtoEvent::Hello`]. Pads with `'?'` if
...
const CERT_FILE_NAME: &str = "monitorhop.pem";
...
fn default_path() -> Result<PathBuf, VarError> {
    ...
    let default_path = {
        ...
        format!("{xdg_config_home}/monitorhop/")
    };
...
struct TomlClient {
    ...
    /// configs → `Auto`. See [`monitorhop_ipc::ConnectionMode`].
    #[serde(default)]
    mode: Option<ConnectionMode>,
    /// Per-network address locks, keyed by network fingerprint. Absent
    /// in legacy configs. See [`monitorhop_ipc::ClientConfig::network_locks`].
    #[serde(default)]
...
struct Args {
    /// the listen port for monitorhop
    #[arg(short, long)]
    port: Option<u16>,
...
pub enum Command {
    ...
    /// MonitorHop commandline interface
    Cli(CliArgs),
    /// run in daemon mode
    Daemon,
    /// Add (or, with --remove, delete) a host-firewall rule allowing
    /// monitorhop's UDP port inbound — for the detected firewall
